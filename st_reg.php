				...
				<?php
					if($_SERVER['REQUEST_METHOD'] == "POST"){
						...
						if(empty($st_id) or empty($st_name) or empty($st_pass ) or empty($st_email) or empty($BirthMonth) or empty($BirthDay) or empty($BirthYear) or empty($st_dept) or empty($st_contact) or empty($st_gender) or empty($st_add)){
							...
							$st_pass = md5($st_pass);
							$st_register = $user->st_registration($st_id,$st_name,$st_pass,$st_email,$bday,$st_dept,$st_contact,$st_gender,$st_add, $sec_question, $sec_answer);
							if($st_register){
		...
		<form action="" method="post" id="st_form">
			<table>
				...
				<tr>
					...
					<td><input type="text" name="sec_question" placeholder="Security Question" required /></td>
				</tr>
				<tr>
